...
public class InsertDonorInfoOperation {
    ...
    {   System.out.println("ghk ");       
        try{
         ...
         Boolean dataSaved = dbc.insertDataToDB("INSERT INTO DONOR(DONORNAME , DonorGender , DonorBloodGroup , DonorAddress , DonorArea,DonorPhoneNumber)"
                                            + " VALUES('"+emp.getDonorName()+"','"+emp.getDonorGender()+"','"+emp.getDonorBloodGroup()
                                                   +"','"+emp.getDonorAddress()+"','"+emp.getDonorArea()+"','"+emp.getDonorPhoneNumber()+"')");
